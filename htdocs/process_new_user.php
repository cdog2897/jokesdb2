...
echo "<h2>Trying to add a new user " . $new_username . " pw =  " . $new_password1 . " and " . $hashed_password . "</h2>";

preg_match('/[0-9]+/', $new_password1, $matches);
if(sizeof($matches) == 0) {
    echo "The password must at least have one number.<br>";
    exit;
}

preg_match('/[!@#$%^&*()]+/', $new_password1, $matches);
if(sizeof($matches) == 0) {
    echo "The password must contain a special character. <br>";
    exit;
}
